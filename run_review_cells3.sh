...
CELLS=(
  # ---- block G, added 8/20 after the CFP moved to 8/24. Limitations says "other
  # cells use a single seed", and the cells that sentence covers are now the two
  # HEADLINE claims: three of the four 2x2 cells already have seeds 0/1/2 on fold
  # 0, but `csl_daily-pose+how2sign-mt5` has only seed 0, and one missing cell
  # blocks the interaction at any seed but 0. Two cells buy the whole seed check.
  # The lm_head decomposition is single-seed throughout, so six more cover it.
  # Fold 0 only, matching how block C reported seed spread.
  "0|csl_daily|pose|how2sign|mt5|1|12|"
  "0|csl_daily|pose|how2sign|mt5|2|12|"
  "0|how2sign|all|csl_daily|lm_head|1|12|"
  "0|how2sign|all|csl_daily|lm_head|2|12|"
  "0|how2sign|all|csl_daily|mt5_nohead|1|12|"
  "0|how2sign|all|csl_daily|mt5_nohead|2|12|"
  "0|csl_daily|all|how2sign|lm_head|1|12|"
  "0|csl_daily|all|how2sign|lm_head|2|12|"
  # ---- block F, added 8/20: the two folds this queue forgot. `mt5_nohead` is the
  # CONTROL ARM of the localisation claim -- lm_head-only against everything-but --
  # and block E filled folds 1 and 2 for both lm_head cells on 8/19 while leaving
  # its control at fold 0, so Table 1 compares 940 items against 330 in one
  # sentence and the arm whose interval has to stay near zero is the one with a
  # third of the data ([+0.0, +6.1], p=0.084). First in the list because it is
  # worth more than anything else outstanding.
  "1|how2sign|all|csl_daily|mt5_nohead|0|12|"
  "2|how2sign|all|csl_daily|mt5_nohead|0|12|"
  # ---- block B tail: the third ASL encoder (in flight when this list was written)
  "2|openasl|pose|csl_daily|mt5|0|12|"
  # ---- block E: lm_head is the decoder effect. Fold 0 says one 192.1M-parameter
  # tensor moves How2Sign from 24.5 to 47.6 while the other 774.5M of the mT5
  # half move it to 27.6, and that the swap collapses CSL-Daily symmetrically
  # (54.8 -> 24.2). These four make it a three-fold row like every other.
  ...
  # ---- block E': does it generalise past the pair it was found on? Two cells,
  # each removing one way of explaining the result away.
  #   openasl + CSL-Daily lm_head   -> is a SECOND ASL checkpoint rescued too, or
  #                                    is How2Sign's head unusually compatible?
  #   how2sign + CSL-News lm_head   -> is it the output language, or is it
  #                                    CSL-Daily's particular head?
  "0|openasl|all|csl_daily|lm_head|0|12|"
  "0|how2sign|all|csl_stage1|lm_head|0|12|"
  ...
  # ---- block A': the third English decoder. Lowest priority: two independently
  # fine-tuned English decoders already agree, and this one is an isolated-sign
  # model whose "written output" is a gloss vocabulary.
...
echo "### review-response sweep (v3, reordered 8/19): ${#CELLS[@]} cells, priority order"
if [ -n "$DRY" ]; then
